/********     ViewRep C Program Source Code File (.C)	       ********/
/*								      */
/* PROGRAM NAME: ViewRep      Application (ViewRep)		      */
/* -------------						      */
...
/* -------------						      */
/*								      */
/* DATE AND TIME: Fri Nov  9 16:32:48 2001			      */
/* -------------						      */
/*								      */
/* COPYRIGHT:							      */
/* ----------							      */
/* (C) 1995 - 2001 Elipse Limied				      */
/*								      */
/* REVISION LEVEL: 1.2						      */
/* ---------------						      */
/*								      */
...
/*  ---------------						      */
/*								      */
/*    ViewRep.C       - Source code				      */
/*    ViewRep.Cmd     - Command file to build this program	      */
/*    ViewRep.Mak     - Make file for this program		      */
/*    ViewRep.Def     - Module definition file			      */
/*    ViewRep.Ext     - External definition file		      */
/*    ViewRep.H       - Application header file 		      */
/*    ViewRep.Ids     - Pm header file				      */
/*    ViewRep.L       - Linker automatic response file		      */
/*    ViewRep.Rc      - Resource file				      */
/*								      */
...
static	VOID ActionActionExit( PGPFPARMS pGpfParms );
static	VOID ActionActionPrint( PGPFPARMS pGpfParms );
static	MRESULT ProcessMainWindow( PGPFPARMS pGpfParms );
static	VOID CommandMainWindow( PGPFPARMS pGpfParms );

/**********************************************************************/
/*								      */
/* Miscellaneous constants ViewRep.H				      */
/*								      */
/**********************************************************************/
...
INT _CENTRY main( LONG argc, PCHAR *argv )
{
 ...
 HPOINTER hptrIcon;		      /* Handle of the Icon	      */
  ...
  hmodViewRep	  = (HMODULE)NULL;   /* hMod Inline		      */
  hwndHelp	  = (HWND)0;	     /* Help Hwnd		      */
  szTaskList	  = (PCHAR)"Report Preview";
  EndOfAppl	  = FALSE;	     /* Switch Accept End Application */
  ViewRep_Level = (PCHAR)"1.2";
  ...
  hinit.hmodHelpTableModule	 = hmodViewRep;
  ...
  hinit.pszHelpLibraryName	 = (PSZ)"ViewRep.Hlp";
...
SHORT APIENTRY GpfSendMsg( SHORT Id )
{
 ...
 /* Load Msg strings						      */
 /*********************************************************************/
 WinLoadString( habMainTask, hmodViewRep,
...
static VOID WmInitWndMainWindow ( PGPFPARMS pGpfParms )
 {
	...
	view_fill( pGpfParms );
	view_title( pGpfParms );
	prof_make_siz_pos( pGpfParms );
	return;
...
/**********************************************************************/
/*								      */
/*   STATIC PRIVATE FUNCTION: ActionActionExit			      */
/*								      */
/*   Takes appropriate action when a WM_COMMAND/WM_CONTROL message is */
/*   received by the window procedure.				      */
/*   Window "MainWindow".					      */
...
/**********************************************************************/
/*								      */
/*   STATIC PRIVATE FUNCTION: ActionActionPrint 		      */
/*								      */
/*   Takes appropriate action when a WM_COMMAND/WM_CONTROL message is */
/*   received by the window procedure.				      */
/*   Window "MainWindow".					      */
...
static VOID ActionActionPrint( PGPFPARMS pGpfParms )
{
  ...
  switch( pGpfParms->Notify )
  {
   case COMMAND:
	/**************************************************************/
	/* Disable Menu/Item "ActionPrint".			      */
	/* Window "MainWindow". 				      */
	/**************************************************************/
	GpfMenuGray( hwndFrameMainWindow, ID_ACTIONPRINT, TRUE );
	/**************************************************************/
	/* Disable Menu/Item "ActionExit".			      */
	/* Window "MainWindow". 				      */
	/**************************************************************/
	GpfMenuGray( hwndFrameMainWindow, ID_ACTIONEXIT, TRUE );
	view_print( pGpfParms );
	return;
  }
  /* Take no action for any other selections			      */
  return;
...
static VOID ActionListBox( PGPFPARMS pGpfParms )
{
  ...
  switch( pGpfParms->Notify )
  {
   case LN_SELECT:     /* Catch click on */
	/**************************************************************/
	/* Reset Selection "ListBox".				      */
	/* Window "MainWindow". 				      */
	/**************************************************************/
	{
	 SHORT ItemID;
	 ItemID = SHORT1FROMMR(WinSendDlgItemMsg(hwndMainWindow,
					   ID_LISTBOX,
					   LM_QUERYSELECTION,
					   MPFROMSHORT(LIT_FIRST),
					   MPFROMSHORT(FALSE)));
	 if ((ItemID != (SHORT)LIT_ERROR) && (ItemID != (SHORT)LIT_NONE))
	     WinSendDlgItemMsg(hwndMainWindow,
			       ID_LISTBOX,
			       LM_SELECTITEM,MPFROMSHORT(ItemID),
			       MPFROMSHORT(FALSE));
	}
	return;
  }
